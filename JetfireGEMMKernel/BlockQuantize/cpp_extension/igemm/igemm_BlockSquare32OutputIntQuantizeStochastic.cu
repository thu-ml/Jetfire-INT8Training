...
__global__ void igemm_output_int_quantize_stochastic_cuda_kernel(
    ...
    const int M, const int N, const int K, unsigned long seed) {
    ...
    for (int x = 0; x < 4; x += 2) {
        for (int y = 0; y < 4; y += 2) {
            ...
            absMaxVal = thread_max[comp_c_frag_m][comp_c_frag_n * 32];
            for (int i = 0; i < 4; i++) {
                int8_t ci_val[8];
                float cf_val[8];
                ...
                float noise_list[8];
                FLOAT4(noise_list[0]) = FLOAT4(curand_uniform4(&state));
                FLOAT4(noise_list[4]) = FLOAT4(curand_uniform4(&state));

                for (int j = 0; j < 8; j++) {
                    ci_val[j] = __float2int_rn(127 * cf_val[j] / absMaxVal + noise_list[j] - 0.5f);
...
std::tuple<torch::Tensor, torch::Tensor> igemm_output_int_quantize_stochastic_cuda(
    ...
    int maxSmem = 30 * 1024;
    cudaFuncSetAttribute(igemm_output_int_quantize_stochastic_cuda_kernel<int8_t, int8_t>, cudaFuncAttributeMaxDynamicSharedMemorySize, maxSmem);

    igemm_output_int_quantize_stochastic_cuda_kernel<int8_t, int8_t><<<gridDim, blockDim, maxSmem>>>(
        X.data_ptr<int8_t>(),
        W.data_ptr<int8_t>(),
