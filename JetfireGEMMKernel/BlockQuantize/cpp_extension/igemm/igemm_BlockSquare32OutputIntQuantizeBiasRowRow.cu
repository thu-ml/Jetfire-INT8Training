...
__device__ int8_t clamprowrow(int value, int8_t minVal, int8_t maxVal) {
...
__global__ void igemm_output_int_quantize_bias_rowrow_cuda_kernel(
    ...
    for (int x = 0; x < 4; x += 2) {
        for (int y = 0; y < 4; y += 2) {
            ...
            absMaxVal = thread_max[comp_c_frag_m][comp_c_frag_n * 32] + biasmax[0];
            for (int i = 0; i < 4; i++) {
                int8_t ci_val[8];
                float cf_val[8];
                float bias_reg[8];
                ...
                for (int j = 0; j < 8; j++) {
                    ...
                    ci_val[j] = clamprowrow(__float2int_rn(127 * (cf_val[j] + bias_reg[j]) / absMaxVal), -127, 127);
...
std::tuple<torch::Tensor, torch::Tensor> igemm_output_int_quantize_bias_rowrow_cuda(
    ...
    int maxSmem = 30 * 1024;
    cudaFuncSetAttribute(igemm_output_int_quantize_bias_rowrow_cuda_kernel<int8_t, int8_t>, cudaFuncAttributeMaxDynamicSharedMemorySize, maxSmem);

    igemm_output_int_quantize_bias_rowrow_cuda_kernel<int8_t, int8_t><<<gridDim, blockDim, maxSmem>>>(
        X.data_ptr<int8_t>(),
        W.data_ptr<int8_t>(),
        bias.data_ptr<float>(),
