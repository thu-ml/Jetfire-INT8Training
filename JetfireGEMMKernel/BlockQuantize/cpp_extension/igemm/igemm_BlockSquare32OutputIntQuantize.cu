...
__global__ void igemm_output_int_quantize_cuda_kernel(
    ...
    for (int x = 0; x < 4; x += 2) {
        for (int y = 0; y < 4; y += 2) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                for (int j = 0; j < 8; j++) {
                    ci_val[j] = __float2int_rn(127 * cf_val[j] / absMaxVal);
...
std::tuple<torch::Tensor, torch::Tensor> igemm_output_int_quantize_cuda(
    ...
    int maxSmem = 30 * 1024;
    cudaFuncSetAttribute(igemm_output_int_quantize_cuda_kernel<int8_t, int8_t>, cudaFuncAttributeMaxDynamicSharedMemorySize, maxSmem);

    igemm_output_int_quantize_cuda_kernel<int8_t, int8_t><<<gridDim, blockDim, maxSmem>>>(
        X.data_ptr<int8_t>(),
        W.data_ptr<int8_t>(),
